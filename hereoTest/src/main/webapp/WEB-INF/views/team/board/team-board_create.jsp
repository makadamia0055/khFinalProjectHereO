				...
				<form id="boardForm" action="<c:url value='/team/board_write'></c:url>" method="post">
					<ul class="list-board_write">
						<li class="title">
							<dl>
								<dt>제목</dt>
								<dd><input type="text" name="bo_title" placeholder="제목 입력" required></dd>
							</dl>
						</li>
						<li class="info">
							<dl>
							...
							<dl>
								<dt>글쓴이</dt>
								<dd><input type="text" name="bo_me_id" placeholder="글쓴이 입력" readonly value="
									<c:if test='${not empty loginUser }'>${loginUser.me_nickname}</c:if>
								"></dd>
							</dl>
						</li>
						<li class="cont">
